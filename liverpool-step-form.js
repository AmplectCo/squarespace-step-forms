(function stepForms() {
  ...
  const FIRST_STEP_DEFAULT_NAME = "FIRST STEP";
  const DEFAULT_ADD_TO_CART_TEXT = "Register";
  const SUBMIT_FORM_API_URL =
    "https://www.liverpoolfccamps.com/api/commerce/shopping-cart/entries";
  const VALIDATE_FORM_API_URL =
    "https://www.liverpoolfccamps.com/api/rest/forms/validate";
  const CART_URL = "https://www.liverpoolfccamps.com/cart";
  const SUBMIT_TEMP_FORM_API_URL =
    "https://6jc2pvygog.execute-api.us-east-1.amazonaws.com/dev";

  const AGE_NOT_ALLOWED_ERROR_TEXT =
    "This player does not meet the age requirements for this camp";
  ...
  function isAgeWithinRange(birthDate, eventStartDate, ageRange) {
    ...
    const hasHadBirthdayThisYear =
      ...
      (eventStart.getMonth() === birth.getMonth() &&
        eventStart.getDate() >= birth.getDate());
  ...
  function createFields(formConfig, currentStep = 0) {
    ...
    formConfig.steps.forEach((step, index) => {
      ...
      const $title = $('<div class="title"></div>').text(step.title);
      const $description =
        !!step.description.length &&
        $('<div class="description"></div>').append(step.description);
      ...
      step.fields.forEach((field) => {
        ...
        const $description =
          ...
          $('<div class="description"></div>').append(field.description);

        const $error = $('<div class="field-error"></div>').hide();
        ...
        if (field.type !== "section") {
          if (field.type === "select") {
            ...
            $input = [
              $(`<input class='field-element ${field.type}'>`).attr({
                type: "text",
                id: field.id,
                name: field.id,
                placeholder: field.placeholder,
            ...
            field.options.forEach((option) => {
              const $option =
                $(`<fieldset data-question="${option}" class="item">
                    <legend class="question">${option}</legend>
                    <div class="option">
                      <label>
                        <input type="radio" name="${field.id
                  }-field-${option}" value="-2" ${field.required && 'required="required"'
                  }> Strongly Disagree
                      </label>
                    </div>
                    <div class="option">
                      <label>
                        <input type="radio" name="${field.id
                  }-field-${option}" value="-1" ${field.required && 'required="required"'
                  }> Disagree
                      </label>
                    </div>
                    <div class="option">
                      <label>
                        <input type="radio" name="${field.id
                  }-field-${option}" value="0" ${field.required && 'required="required"'
                  }> Neutral
                      </label>
                    </div>
                    <div class="option">
            ...
            $input = $(`<fieldset id="${field.id}">
                <div class="field text two-digits" ${!field.showCountryCode && 'style="display:none"'
                  ...
                  <label class="caption">
                    <input class="field-element" x-autocompletetype="phone-country-code" type="text" maxlength="5" data-title="Country" ${field.showCountryCode &&
              field.required &&
              'required="required"'
              }>
                    <span class="caption-text">Country</span>
      ...
      const $btnsWrapper = $stepDiv
        .append('<div class="step-buttons"></div>')
        .find(".step-buttons");

      if (index > 0) {
        const $backButton = $(
          '<button type="button" class="step-button step-button_bordered prev-step-button">Back</button>'
        );
        $btnsWrapper.append($backButton);
      }

      const $nextButton = $(
        '<button type="button" class="step-button next-step-button">Next</button>'
      );
  ...
  function stepForm($addToCart) {
    ...
    function createForm() {
      ...
      $(".next-step-button").on("click", () => {
        apiValidateStep().then(() => {
          ...
          document.dispatchEvent(
            new CustomEvent("formStepLeaved", {
              detail: {
      ...
      $(".step-add-to-cart").on("click", () => {
        apiValidateStep().then(() => {
          document.dispatchEvent(
            new CustomEvent("addToCartFormSubmit", {
              detail: {
                formName: parsedConfig.additionalFieldsForm.name || "",
                paymentOption:
                  (selectedVariation &&
                    selectedVariation.attributes &&
                    Object.values(selectedVariation.attributes)[0]) ||
                  "",
    ...
    function getFieldValue(field) {
      ...
      if (field.type === "checkbox") {
        ...
        const inputs = $(
          `fieldset#${field.id} input, fieldset#${field.id} select`
        );
        const fieldValue = [];
    ...
    function isBirthDateValid(id) {
      const dateBirthArr = Array.from($(`fieldset#${id} input`)).map(
        (input, i) => {
          let numbers = (i < 2 && 2) || 0;
      ...
      const dateBirthDate = new Date(
        `${dateBirthArr[2]}/${dateBirthArr[1]}/${dateBirthArr[0]}`
      );
      ...
      return new Promise((resolve, reject) => {
        $.ajax({
          ...
          complete: function (res, status) {
            ...
            for (let key in errors) {
              const $formItem = $(
                `.step.active .form-item:has(#${key}, [name="${key}"])`
              );
    ...
    function init() {
      $($addToCart).after(
        $("<button>", {
          text:
            $($addToCart).find(".sqs-add-to-cart-button-inner").text() ||
            DEFAULT_ADD_TO_CART_TEXT,
          class:
            "new-add-to-cart-button sqs-suppress-edit-mode sqs-editable-button sqs-button-element--primary",
          click: function () {
            const $selectedVariant = $($addToCart)
              .closest(".product-block")
              .find(".product-variants select");
            if ($selectedVariant.length) {
              ...
              selectedVariation = parsedConfig.variants.find((vara) =>
                Object.values(vara.attributes).includes(value)
              );
            }
            ...
            $("#dynamicPopupLabel").text(
              parsedConfig.additionalFieldsForm.name
            );
